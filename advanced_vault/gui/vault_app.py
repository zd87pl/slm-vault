...
try:
    from shell_components import (
        build_status_badge,
        build_surface_card,
        simple_metric_card,
    )
except ImportError:
    build_status_badge = None
    build_surface_card = None
...
class VaultApp:
    ...
    def _simple_metric_card(self, label: str, value: str) -> ft.Container:
        ...
        if simple_metric_card is None:
            raise RuntimeError("shell_components module is unavailable")
        return simple_metric_card(label=label, value=value)
    ...
    def _show_workspace_view(self, initial_question: Optional[str] = None):
        ...
        if render_workspace_view is None:
            raise RuntimeError("demo_shell module is unavailable")
        return render_workspace_view(self, initial_question=initial_question)

    def _show_library_view(self):
        """Render the Figma-aligned Library view."""
        if render_library_view is None:
            raise RuntimeError("demo_shell module is unavailable")
        return render_library_view(self)

    def _show_connections_view(self):
        """Render the Figma-aligned Connections view."""
        if render_connections_view is None:
            raise RuntimeError("demo_shell module is unavailable")
    ...
    def _show_security_view(self):
        ...
        if render_security_view is None:
            raise RuntimeError("demo_shell module is unavailable")
        return render_security_view(self)
